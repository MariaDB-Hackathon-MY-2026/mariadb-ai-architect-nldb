...
_DEBUG_LOG_PATH = str(Path(__file__).resolve().parents[1] / "debug-d75481.log")
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "pre-fix",
                            "hypothesisId": "H2",
                            "location": "core/db_executor.py:DbConfig.from_env",
                            "message": "Reading DB env",
                            "data": {
                                "dotenv_path": str(_DOTENV_PATH),
                                "dotenv_exists": _DOTENV_PATH.exists(),
                                "cwd": os.getcwd(),
                                "has_MARIADB_HOST": os.getenv("MARIADB_HOST") is not None,
                                "has_MARIADB_PORT": os.getenv("MARIADB_PORT") is not None,
                                "has_MARIADB_USER": os.getenv("MARIADB_USER") is not None,
                                "has_MARIADB_DATABASE": os.getenv("MARIADB_DATABASE") is not None,
                ...
                json.dumps(
                    {
                        ...
                        "runId": "pre-fix",
                        "hypothesisId": "H1",
                        "location": "core/db_executor.py:get_connection",
                        "message": "Attempting DB connect",
                        "data": {
                            "host": cfg.host,
                            "port": cfg.port,
                            "user": cfg.user,
                            "database": database if database is not None else cfg.database,
                            "env_plugin_dir": os.getenv("MARIADB_PLUGIN_DIR"),
                            "env_auth_plugin": os.getenv("MARIADB_AUTH_PLUGIN"),
                            "plugin_dir_param": plugin_dir,
                            "cwd": os.getcwd(),
                    ...
                    json.dumps(
                        {
                            ...
                            "data": {"exc": str(exc), "exc_type": exc.__class__.__name__},
                            "timestamp": int(time.time() * 1000),
                        }
                    )
                    + "\n"
                ...
                json.dumps(
                    {
                        ...
                        "runId": "pre-fix",
                        "hypothesisId": "H3",
                        "location": "core/db_executor.py:runtime_db_config",
                        "message": "Set runtime DB config",
                        "data": {"host": host, "port": int(port), "user": user, "database": database},
                        "timestamp": int(time.time() * 1000),
                    }
                )
                + "\n"
                    ...
                    json.dumps(
                        {
                            ...
                            "hypothesisId": "H3",
                            "location": "core/db_executor.py:runtime_db_config",
                            "message": "Reset runtime DB config",
                            "data": {},
                            "timestamp": int(time.time() * 1000),
                        }
                    )
